...
Codex 플러그인은 marketplace를 통해 설치합니다. 이 저장소에는 `./plugins/genie-goose`를 가리키는 repo-local marketplace 파일 `.agents/plugins/marketplace.json`이 포함되어 있으므로, 별도 복사 없이 Codex에서 바로 genie-goose를 테스트할 수 있습니다.
...
Codex에서도 세션 시작 시 자동 라우팅을 원하면 기존 `plugins/genie-goose/hooks/session-start` 스크립트를 `~/.codex/hooks.json` 또는 `<repo>/.codex/hooks.json`에 연결해야 합니다. Codex 공식 문서는 훅을 플러그인 매니페스트가 아니라 Codex 설정 레이어 옆에서 읽는다고 안내합니다.
